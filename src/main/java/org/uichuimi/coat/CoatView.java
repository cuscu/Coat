...
public class CoatView {
	...
	/**
	 * Adds a message to the log. Messages will be show in the bottom line of the application, and
	 * will be also available in a history window. This method is run in the JavaFx main thread, so
	 * there is no need to encapsulate it in a <code>Platform.runLater()</code> block.
	 *
	 * @param message
	 * 		What you want to say
	 * @param level
	 * 		"info", "warning", "error" or "success", but it is possible to use any String
	 */
	...
	private static void setMessageLabel(String message, String date, Label label, String type) {
		Platform.runLater(() -> {
			if (AVAILABLE_MESSAGE_TYPES.contains(type)) {
				...
				label.setGraphic(new SizableImageView("/img/black/" + type + ".png", SizableImageView.SMALL_SIZE));
	...
	private void assignMenuIcons() {
		openFileMenu.setGraphic(new SizableImageView("/img/black/open.png", SizableImageView.MEDIUM_SIZE));
		saveFileMenu.setGraphic(new SizableImageView("/img/black/save.png", SizableImageView.MEDIUM_SIZE));
	}
	...
	private boolean isVCF(File f) {
		return f.getName().endsWith(".vcf") || f.getName().endsWith(".vcf.gz");
	}
...
//        }
	...
	public void openVcfFile(List<Variant> variantSet, File f) {
