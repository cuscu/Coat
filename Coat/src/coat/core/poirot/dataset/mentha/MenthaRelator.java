...
public class MenthaRelator implements Relator {
    ...
    public void expand(Pearl pearl, PearlDatabase database) {
        ...
        this.pearl = pearl;
        try {
            menthaDataset.getInstances(pearl.getName(), 1).forEach(instance -> createRelationship(instance, 2));
            menthaDataset.getInstances(pearl.getName(), 2).forEach(instance -> createRelationship(instance, 1));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
