...
public class LightVariantsTable extends VBox {
    ...
    private Label warningLabel = new Label("This table will show maximum 5000 results");
    ...
    @NotNull
    private TableColumn<VariantContext, String> createInfoColumn(VCFInfoHeaderLine infoHeaderLine) {
        final TableColumn<VariantContext, String> column = new TableColumn<>(infoHeaderLine.getID());
        column.setCellValueFactory(param -> {
            final Object value = param.getValue().getAttribute(infoHeaderLine.getID());
            return new SimpleObjectProperty<>(value == null ? VCFConstants.EMPTY_ID_FIELD : value.toString());
        });
        column.setVisible(infoHeaderLine.getKey().matches("GNAME|SYMBOL"));
        return column;
    }
